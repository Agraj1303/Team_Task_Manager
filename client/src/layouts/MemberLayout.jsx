...
import { LayoutDashboard, FolderKanban, LogOut, Moon, Sun, Menu, X, User, CheckSquare, FileText } from 'lucide-react';
...
const MemberLayout = () => {
  ...
  const navItems = [
    ...
    { name: 'Reports', path: '/member/reports', icon: FileText },
  ];
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-30 flex flex-col w-72 h-full bg-white/80 backdrop-blur-2xl border-r border-slate-200/60 dark:bg-slate-900/80 dark:border-slate-800/60 transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 shadow-2xl md:shadow-none`}>
        <div className="flex items-center justify-between px-6 py-8">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 to-teal-600 dark:from-emerald-400 dark:to-teal-400 tracking-tight">TaskFlow</h2>
          </div>
        ...
        <div className="px-6 mb-4">
          <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Member Panel</div>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all duration-200 group ${isActive ? 'bg-emerald-50/80 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-semibold shadow-sm shadow-emerald-100 dark:shadow-none' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-slate-200 font-medium'}`}
              >
                <Icon size={20} className={isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors'} />
                <span>{item.name}</span>
              </Link>
            );
          })}
        ...
        <div className="p-4 m-4 mt-auto rounded-2xl bg-slate-50/80 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-700/50 backdrop-blur-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold mt-0.5">Team Member</span>
            </div>
          </div>
      ...
      <div className="flex flex-col flex-1 w-full overflow-hidden relative">
        <header className="flex items-center justify-between px-6 lg:px-10 py-5 bg-slate-50/80 backdrop-blur-xl border-b border-slate-200/60 dark:bg-[#0B1120]/80 dark:border-slate-800/60 z-10 sticky top-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-slate-800 dark:text-white hidden sm:block">My Workspace</h1>
          </div>
